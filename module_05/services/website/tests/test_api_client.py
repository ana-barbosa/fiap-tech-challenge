...
from api_client import CrmUnavailableError, get_property, list_properties, photo_url  # noqa: E402
...
def test_list_properties_returns_json_on_success():
    ...
    with patch("api_client.requests.get", return_value=fake_response) as mocked_get:
...
def test_list_properties_raises_crm_unavailable_on_connection_error():
    with patch("api_client.requests.get", side_effect=requests.ConnectionError("boom")):
        try:
            list_properties()
