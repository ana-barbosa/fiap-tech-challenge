...
def test_push_message_posts_chat_id_and_text_and_returns_true():
    mock_response = MagicMock(status_code=200)
    with patch("src.telegram_bot_client.requests.post", return_value=mock_response) as mock_post:
        result = telegram_bot_client.push_message("123", "Oi de novo!")

    assert mock_post.call_args.kwargs["json"] == {"chat_id": 123, "text": "Oi de novo!"}
    assert result is True


def test_push_message_returns_false_on_request_exception():
...
def test_push_message_returns_false_on_non_numeric_chat_id():
    with patch("src.telegram_bot_client.requests.post") as mock_post:
        result = telegram_bot_client.push_message("seed-66", "Oi de novo!")
